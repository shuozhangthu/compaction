...
hold on
plot(fliplr(C_f(N-81,1:J-81)),z(1+81:end),'--','linewidth',2)
plot(fliplr(C_f(N-165,1:J-165)),z(1+165:end),':','linewidth',2)
plot(fliplr(C_f(N-248,1:J-248)),z(1+248:end),'-.','linewidth',2)
...
set(gca,'FontSize',12)
legend('t=39.8 m.y.','t=35 m.y.','t=30 m.y.','t=25 m.y.','Sr^{2+} Data','location','SouthWest')
print('Sr803.jpg','-djpeg','-r600');
...
title('Sulfate in pore water (site: 803)')
xlabel('Sulfate (mM)');
ylabel('Depth (m)')
set(gca,'FontSize',12)
...
title('Calcium in pore water (site: 803)')
xlabel('Calcium (mM)');
ylabel('Depth (m)')
set(gca,'FontSize',12)
...
save('data803.mat', '-struct', 'S'); 

save('seawater803.mat','seawater803');