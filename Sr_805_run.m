...
for n = 1:N
    for j = 1:n
        R_net(n,j) = 1*(y805(2)+y805(3)*exp(-z(n-j+1)/y805(4)));
    end
end
...
L = 0.5;
k_su = x805(3);
G0 = x805(2);  %[mM/1 porewater]  by fitting
...
load lear_curve.mat
seawater805=transpose(lear_curve(t)).*Ca_sea/1000;
...
hold on
plot(fliplr(C_f(N-206,1:J-206)),z(1+206:end),'--','linewidth',2)
plot(fliplr(C_f(N-336,1:J-336)),z(1+336:end),':','linewidth',2)
plot(fliplr(C_f(N-465,1:J-465)),z(1+465:end),'-.','linewidth',2)
scatter(Sr/1000,depth,'ko')      % convert from muM to mM
title('Site 805');
xlabel('Sr in pore water (mM)');
ylabel('Present-day depth (m)')
set(gca,'FontSize',12)
legend('t=27.9 m.y.','t=20 m.y.','t=15 m.y.','t=10 m.y.','Sr^{2+} Data','location','SouthWest')
print('Sr805.jpg','-djpeg','-r600');

%---------
figure;
plot(fliplr(C_su(N,1:J)),z(1:N),'linewidth',2)
hold on
plot(fliplr(C_su(N-81,1:J-81)),z(1+81:end),'--','linewidth',2)
plot(fliplr(C_su(N-165,1:J-165)),z(1+165:end),':','linewidth',2)
plot(fliplr(C_su(N-248,1:J-248)),z(1+248:end),'-.','linewidth',2)
set(gca,'Ydir','reverse')
title('Sulfate in pore water (site: 805)')
xlabel('Sulfate (mM)');
ylabel('Depth (m)')
set(gca,'FontSize',12)
...
legend('t=39.8 m.y.','t=35 m.y.','t=30 m.y.','t=25 m.y.','Sulfate Data')

% ---------
figure
plot(fliplr(Ca_pore(N,1:J)),z,'linewidth',2)
hold on
plot(fliplr(Ca_pore(N-81,1:J-81)),z(1+81:end),'--','linewidth',2)
plot(fliplr(Ca_pore(N-165,1:J-165)),z(1+165:end),':','linewidth',2)
plot(fliplr(Ca_pore(N-248,1:J-248)),z(1+248:end),'-.','linewidth',2)

set(gca,'Ydir','reverse')
scatter (Ca2_data,depth2,'ko');
title('Calcium in pore water (site: 805)')
xlabel('Calcium (mM)');
ylabel('Depth (m)')
set(gca,'FontSize',12)

legend('t=39.8 m.y.','t=35 m.y.','t=30 m.y.','t=25 m.y.','Calcium Data')
...
save('data805.mat', '-struct', 'S'); 

save('seawater805.mat','seawater805');
